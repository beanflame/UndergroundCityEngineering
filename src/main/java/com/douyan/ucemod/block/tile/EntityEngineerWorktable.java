...
import com.douyan.ucemod.inventory.UCEContainer;
...
public class EntityEngineerWorktable extends TileEntity implements ITickable
{
	...
	public SlotItemHandler slot2 = new SlotItemHandler(items,1,44,32){
        @Override
        public boolean isItemValid(@Nonnull ItemStack stack) {
            return stack.getItem()== Item.getItemFromBlock(UCEBlock.MINING_MACHINE);
        }
    };
	...
	public EntityEngineerWorktable()
    {
    }
    ...
    public void update()
    {
        ...
        if(world!=null&&!world.isRemote){
            if(slot1.getHasStack()&&slot2.getHasStack()&&(!slot3.getHasStack()||slot3.getStack().getCount()<64)){
                if(cook<=0)setCookTime(UCEContainer.MAX_TIME);
                updateCookTime();
                if(cook<=0){
                    //完成制作
                    ...
                    else slot3.putStack(new ItemStack(UCEBlock.CORE_BLOCK));
                    clearCookTime();
                }
            }else{
                clearCookTime();
